...
**NB there is an issue with the frequent ED attenders activity subset such that it is not possible to identify instances in 2011 or 2012. As such, to ensure that the trends are not affected by this data quality issue, in the analysis for the ED attendances point of delivery, we remove these two initial years so that the analysis runs from 2013-2019.**
...
hospitalActivity_Df <- read.csv(
  "data/nonDemogMitig_data_SplitZeroLosIpEmer_240318.csv")
...
pod_lengthener_fn <- function(df) {
  df |> mutate(
    pod_long = case_when(pod == "IpElec" ~ "Elective admission",
                         pod == "IpEmer_zeroLoS" ~ "Non-elective zero LoS admission",
                         pod == "IpEmer_nonZeroLoS" ~ "Non-elective non-zero LoS admission",
    ...
    mutate(pod_long = factor(pod_long, 
                             levels = c("Elective admission",
                                        "Non-elective zero LoS admission",
                                        "Non-elective non-zero LoS admission",
                                        "Maternity admission",
...
hospitalActivity_Df_adj <- hospitalActivity_Df |>
  filter(!(pod == "edAtt" & between(yr,2011,2012)))
...
# add in the population_share variable
...
fit_and_extract_am <- function(df, df_name) {
  # Fit the GAM model
  model <- gam(activity ~s(age, by = sex_mitigatable) + sex + mitigatable + year + year:mitigatable + offset(log(population)),
               family = nb(link = "log"),
  ...
  interaction_rows <- coefs |> 
    filter(term %in% c("mitigatableprev:year",
...
hospitalActivity_Df_adj |>
  filter(pod %in% c("IpEmer_nonZeroLoS", "IpEmer_zeroLoS")) |>
  #pod_lengthener_fn() |>
  mitigator_lengthener_fn() |>
  group_by(yr, pod, mitigatable_long) |>
  summarise(activity = sum(activity)) |> 
...
activity_mitigators_gam_data |>
  filter(year==2019, pod %in% c("IpEmer_nonZeroLoS", "IpEmer_zeroLoS")) |>
  #pod_lengthener_fn() |>
  mutate(pod = stringr::str_wrap(pod, width = 10)) |>
  mitigator_lengthener_fn() |>
  group_by(age, pod, mitigatable_long) |>
  summarise(activity = sum(activity),
...
Activity in the purview of de-adoption grew at a slower rate than usual activity by -2.11% (-2.95% to -1.26%) for elective admissions and -2.57% (-2.82% to -2.32%) for outpatient attendances per annum. ED attendances in the purview of redirection/substitution grew at a slower rate, by -0.39% (-0.74% to -0.04%) per annum than usual activity. Elective and 1+ day non-elective admissions in the purview of prevention grew at a faster rate than usual activity, by 2.17% (1.29% to 3.06%) and 1.09% (-0.44% to 1.75%) per annum respectively, whilst zero-day non-elective admissions grew at a slower rate than usual activity, by -2.98% (-3.73% to -2.24%). Non-elective admissions in the purview of redirection/substitution grew at a faster rate, by 1.72% (1.07% to 2.37%) and 1.02% (0.26% to 1.79%) per annum for non-zero and zero day admissions than usual activity.
