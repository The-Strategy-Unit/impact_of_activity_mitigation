---
title: "Exploration of activity mitigation data"
format: 
  html:
...
The activity mitigation analysis is being updated. As part of that, we have moved to using Data Bricks to extract the data. This is about comparing the results to one another. There may be changes to do with updated logic for flagging mitigators and various differences in inclusions and exclusions.

The other key difference is that for this analysis we are bringing in data on diagnosis. (These are ICD10 chapter for APC; ambulance/walk-in for ED; surgical / non-surgical / maternity for outpatient.)
...
opa_mitigators_data_201112_201415 <- read.csv(
  here(
    "2025 analysis", 
    "Data Bricks results and queries", 
    "2025_03_27_opa_mitig_data_201112-201415.csv"
    )
  )
...
opa_mitigators_data_201516_201819 <- read.csv(
  here(
    "2025 analysis", 
    "Data Bricks results and queries", 
    "2025_03_27_opa_mitig_data_201516-201819.csv"
    )
  )
...
opa_mitigators_data_201920 <- read.csv(
  here(
    "2025 analysis", 
    "Data Bricks results and queries", 
    "2025_03_27_opa_mitig_data_201920.csv"
    )
  )
...
mitigators_data_previous <- mitigators_data_previous |> 
  #filter(pod %in% c("IpElec", "IpMat", "IpEmer")) |> 
...
We can see here that the activity data doesn't have the region variable populated prior to 2015/16.

**TO-DO: investigate using the local authority variable `RESLADST` to get the region of the patient. It is populated.**
...
<!-- Across the study period, we can see that the most common chapters for emergency admissions are *X - Diseases of the respiratory system* (12.4%), *XVIII - Symptoms, signs and abnormal clinical and laboratory findings, not elsewhere classified* (16.3%) and *XIX - Injury, poisoning and certain other consequences of external causes* (12.2%). -->

<!-- For elective, they are *II - Neoplasms* (22.3%), *XI - Diseases of the digestive system* (15.5%) and *XIV - Diseases of the genitourinary system* (13.0%). -->

<!-- *XV - Pregnancy, childbirth and the puerperium* is unsurprisingly by far the most common (85.8%) for maternity admissions. Almost all of the remaining (12.8%) are *XXI - Factors influencing health status and contact with health services* -->

The ICD10 chapters are printed in the annex.
...
We can see the activity rate by IMD quintile in 2019 below.
...
current_mitigators_population_data |> 
  ...
  ggtitle("Activity rate by IMD quintile and pod, 2019")
```
## And activity rates by region (one per pod, exc. maternity)
...
current_mitigators_population_data |> 
  ...
  facet_wrap(~pod, scales = "free_y") +
  ggtitle("Activity rates by region and IMD quintile") +
