...
purrr::map_dfr(
  list(
    ...
    "outpatients" = opa_bam_re, 
    "ED" = ed_bam_re), 
  ~broom::tidy(., parametric = TRUE),
  .id = "pod") |> 
  filter(str_ends(term, ":fyear")) |> 
  mutate(mitigation_type = str_extract(term, "(?<=mitigation_type)(.*?)(?=:fyear)"),
         growth = exp(estimate),
         lower_ci = exp(estimate - 1.96 * std.error),
         upper_ci = exp(estimate + 1.96 * std.error)) |> 
  dplyr::select(pod, mitigation_type, growth, lower_ci, upper_ci) |> 
  gt::gt()
...
age_sex_standardised_rates |> 
  filter(!(pod=="edAtt" & fyear == 2019)) |> 
  ggplot(aes(x=fyear, y=expected_activity, colour = mitigation_type)) +
  geom_line(linewidth=1) +
  scale_y_continuous(labels = scales::label_number(suffix = "m", scale = 1e-6)) +
  scale_x_continuous(breaks = scales::pretty_breaks()) +
  facet_wrap(~pod, scales = "free_y") +
