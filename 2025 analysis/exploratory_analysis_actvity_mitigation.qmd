...
opa_mitigators_data_201112_201415 <- read.csv(
  here(
    "2025 analysis", 
    "Data Bricks results and queries", 
    "2025_04_16_opa_mitig_data_inc_tele_201112-201415.csv"
    )
  )
...
opa_mitigators_data_201516_201819 <- read.csv(
  here(
    "2025 analysis", 
    "Data Bricks results and queries", 
    "2025_04_16_opa_mitig_data_inc_tele_201516-201819.csv"
    )
  )
...
opa_mitigators_data_201920 <- read.csv(
  here(
    "2025 analysis", 
    "Data Bricks results and queries", 
    "2025_04_16_opa_mitig_data_inc_tele_201920.csv"
    )
  )
...
-   The drop is from 8,231,327 to 7,808,138 which equates to 423,189 fewer rows
-   There are 422,561 rows in the activity data for which the region variable is not one of the 9 Government Office Regions, thus the vast majority of rows which don't pass through the join are due to that.
:::
...
::: callout-note
We use big additive models (BAMs) instead of general additive models (GAMs) in this analysis given the very large amounts of data being analysed - there are 4.4m rows in the linked data.
...
elective_bam_re <- bam(
  activity ~ s(age, by = sex_icd) + sex + mitigation_type + fyear + fyear:mitigation_type + chapter_number + imd_quintile + s(resgor_ons, bs = "re") + offset(log(population)),
  family = nb(link = 'log'),
...
bams <- list(
  "IpElec" = elective_bam_re,
  "IpEmer" = non_elective_bam_re,
  "opAtt" = opa_bam_re,
  "edAtt" = ed_bam_re
)
...
previous_gam_results |> 
  mutate(
    `Point of delivery` = factor(
      `Point of delivery`,
      levels = c("Elective admission", "Non-elective admission", "Outpatient attendance", "ED attendance")
      )
    ) |> 
  arrange(`Point of delivery`, desc(Mitigation)) |> 
...
pods <- list("IpElec", "IpEmer", "opAtt", "edAtt")
...
names(previous_gams_with_new_data) <- list(
  "IpElec",
  "IpEmer",
  "opAtt",
  "edAtt")
...
For comparison we present standardised rates of each mitigation type and pod in a chart. The standardisation is using the baseline in 2015/16 as the reference population, controlling for age, sex, region and deprivation quintile.
