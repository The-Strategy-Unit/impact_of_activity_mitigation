...
apc_mitigators_data <- read.csv(
  here(
    ...
    "apc_mitig_data_exc_post_ranking.csv"
    )
  )
```
...
results_by_activity_subset <- read.csv(
  here::here(
    "2025 analysis",
    "Data Bricks results and queries",
    "activity_by_strategy_breakdown_exc_post_ranking.csv"
  )
)
...
mitigatable_totals <- current_mitigators_data |> 
  filter(fyear %in% c(2011, 2019)) |> 
  filter(mitigation_type != "none") |> 
  summarise(activity = sum(activity), .by = c(fyear, pod)) |> 
  pod_lengthener_fn() |> 
  mutate(mitigatable_long = "Mitigatable")

# Combine into full_table
full_table <- bind_rows(activity_subset_table, totals, mitigatable_totals)
...
full_table <- full_table |> 
  mutate(activity_subset = case_when(
    mitigatable_long == "All activity" ~ "All activity",
    mitigatable_long == "Mitigatable" ~ "All mitigatable activity",
...
gt_table <- pivot_table_pct |> 
  ...
  tab_style(
    ...
    locations = cells_body(
      rows = mitigatable_long %in% c("All activity", "Not mitigatable", "Mitigatable")
    )
  ) |> 
  tab_options(
...
The table below shows the count and proportion of activity by ICD-10 chapter for each pod in 2011/12. We have the yearly figures in the annex.
...
apc_mitigators_data |>
  ...
  summarise(activity = sum(activity), .by = c(pod_long, chapter_number, Title)) |>
  mutate(
    proportion = activity / sum(activity), .by = c(pod_long),
  ...
  pivot_wider(names_from = pod_long, values_from = c(activity, proportion), names_glue = "{pod_long}_{.value}") |> 
  ...
  tab_spanner(
    label = "Elective",
    columns = matches("^Elective admissions")
  ) |>
  tab_spanner(
    label = "Non-elective",
    columns = matches("^Non-elective admissions")
  )
...
::: callout-note
The population data is perfectly filled in for all combinations of `year` (2011/12 - 2019/20), `age` (1-90), `sex` ("f" or "m"),  `rgn09cd` ("E12000001"-"E12000009"), and `imd_quintile` (1-5).

The activity data was extracted in such as a way as to remove any NA values in the joining columns.

There are `r scales::comma(nrow(current_mitigators_data))` rows in `current_mitigators_data` prior to inner joining onto the population data and `r scales::comma(nrow(current_mitigators_population_data))` in the joined data.

**Therefore the removal of NAs worked as expected and we are losing no rows from the exploratory analysis when we feed these data into our regression formulae.**
:::
